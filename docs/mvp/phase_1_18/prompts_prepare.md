# Промпт: ПОДГОТОВКА — TASK-4.4: scheduler-service (шаблон)
...
### Описание
Дополнить: ScheduledJobEntity, Bull Queue stubs (passive-income, job-reset, shop-rotation, leaderboard), scheduler.http.controller.ts.
...
1. В `docs/mvp/phase_1_18/task.md` замени статус на `PREPARING`.
2. Добавь запись в лог прогресса: дата, «Подготовка начата», имя агента.
3. В `docs/mvp/mvp_phase_1.md` обнови статус задачи 18 на `🔄 PREPARING`.

### Шаг 6: Коммит подготовки
```bash
git add docs/mvp/
git commit -m "chore(mvp): prepare TASK-4.4 - scheduler-service (шаблон)
...
- docs/architecture/12_scheduler_service.md
