# Промпт: РЕВЬЮ — TASK-2.1: Auth Service
...
**Тесты:**
- [ ] Unit-тесты покрывают: RegisterHandler, LoginHandler, TokenService, UserEntity, UserPrismaMapper
- [ ] Integration-тесты: POST /auth/register, POST /auth/login, POST /auth/refresh pipeline
...
1. В `docs/mvp/phase_1_5/task.md` замени статус на `IN REVIEW`.
2. В `docs/mvp/mvp_phase_1.md` обнови статус задачи 5 на `👁️ IN REVIEW`.
...
**Задача:** docs/mvp/phase_1_5/task.md

Co-Authored-By: Oz <oz-agent@warp.dev>"
```

### Шаг 6: После мержа
1. В `docs/mvp/phase_1_5/task.md` замени статус на `DONE`.
2. В `docs/mvp/mvp_phase_1.md` обнови статус задачи 5 на `✅ DONE`.
