# Промпт: ПОДГОТОВКА — TASK-4.7: history-service (шаблон)
...
### Описание
Шаблон: MatchHistoryEntity, ReplayEntity, IMatchHistoryRepository, history.http.controller.ts, history.nats.controller.ts.
...
1. В `docs/mvp/phase_1_21/task.md` замени статус на `PREPARING`.
2. Добавь запись в лог прогресса: дата, «Подготовка начата», имя агента.
3. В `docs/mvp/mvp_phase_1.md` обнови статус задачи 21 на `🔄 PREPARING`.

### Шаг 6: Коммит подготовки
```bash
git add docs/mvp/
git commit -m "chore(mvp): prepare TASK-4.7 - history-service (шаблон)
...
- docs/architecture/29_match_history_system.md
