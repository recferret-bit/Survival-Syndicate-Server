# Промпт: ПОДГОТОВКА — TASK-4.5: collector-service (шаблон)
...
### Описание
Шаблон без domain. IClickHouseRepository, ClickHouseService/Module, analytics.nats.controller.ts (subjects из @lib/lib-analytics).
...
```bash
git worktree add ../worktrees/chore-scaffold-collector-service -b phase_1_19/chore/scaffold/collector-service main
cd ../worktrees/chore-scaffold-collector-service
```
Или без worktree:
```bash
git checkout -b phase_1_19/chore/scaffold/collector-service main
```
...
1. В `docs/mvp/phase_1_19/task.md` замени статус на `PREPARING`.
2. Добавь запись в лог прогресса: дата, «Подготовка начата», имя агента.
3. В `docs/mvp/mvp_phase_1.md` обнови статус задачи 19 на `🔄 PREPARING`.

### Шаг 6: Коммит подготовки
```bash
git add docs/mvp/
git commit -m "chore(mvp): prepare TASK-4.5 - collector-service (шаблон)
...
- docs/architecture/03_analytics_and_events.md
