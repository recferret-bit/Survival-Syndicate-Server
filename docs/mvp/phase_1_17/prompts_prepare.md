# Промпт: ПОДГОТОВКА — TASK-4.3: combat-progress-service (шаблон)
...
### Описание
Полный шаблон: PlayerProgressEntity, BattlePassEntity, AchievementEntity, ports, все модули, stub controllers.
...
1. В `docs/mvp/phase_1_17/task.md` замени статус на `PREPARING`.
2. Добавь запись в лог прогресса: дата, «Подготовка начата», имя агента.
3. В `docs/mvp/mvp_phase_1.md` обнови статус задачи 17 на `🔄 PREPARING`.

### Шаг 6: Коммит подготовки
```bash
git add docs/mvp/
git commit -m "chore(mvp): prepare TASK-4.3 - combat-progress-service (шаблон)
...
- docs/architecture/10_combat_progress_service.md
