# Промпт: РЕВЬЮ — TASK-1.2: Docker Compose
...
**Тесты:**
- [ ] Unit-тесты покрывают: Нет (инфраструктурная задача)
- [ ] Integration-тесты: docker compose up проходит без ошибок
...
1. В `docs/mvp/phase_1_2/task.md` замени статус на `IN REVIEW`.
2. В `docs/mvp/mvp_phase_1.md` обнови статус задачи 2 на `👁️ IN REVIEW`.
...
**Задача:** docs/mvp/phase_1_2/task.md

Co-Authored-By: Oz <oz-agent@warp.dev>"
```

### Шаг 6: После мержа
1. В `docs/mvp/phase_1_2/task.md` замени статус на `DONE`.
2. В `docs/mvp/mvp_phase_1.md` обнови статус задачи 2 на `✅ DONE`.
