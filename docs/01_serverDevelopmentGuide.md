...
## Часть 12: Архитектура Игрового Сервера (Real-time)

Игровой сервер — это отдельный класс сервисов, работающий в режиме реального времени через WebSocket. Его архитектура принципиально отличается от обычных микросервисов.
...
6.  **Основные Компоненты**
    ...
    -   **GameLoop:** Сердце сервера. Работает с фиксированной частотой.
...
1.  **Центральный сервер + Региональные кластеры**
    -   **Центральный сервер (Москва):** Сервисы, не требующие низкой задержки: Auth, Matchmaking, Meta API, а также основные базы данных (PostgreSQL, ClickHouse).
    -   **Региональные кластеры (VPS в разных городах):** Только сервисы реального времени: WebSocket Gateway и Game Server Pods. Находятся близко к игрокам.

2.  **WebSocket Gateway как прокси**
    -   WebSocket Gateway — единственная точка входа для всех WebSocket-соединений игроков в данном регионе.
    -   Он **не содержит** игровой логики, а только проксирует сообщения между клиентами и Game Server Pods через NATS.
    -   **Преимущество:** Игровая логика спрятана внутри приватной сети кластера. Наружу смотрит только stateless Gateway.

3.  **Server Heartbeat Статусы (через NATS)**
    -   Каждый Game Server Pod раз в 5 секунд публикует (broadcast) своё состояние в тему `server.status`.
