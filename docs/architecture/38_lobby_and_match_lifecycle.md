...
5.  **Старт матча:** Когда все игроки нажали "Готов", хост может запустить игру. `Matchmaking Service` запускает стандартный процесс поиска Зоны и передает игроков в `Local Orchestrator`.
